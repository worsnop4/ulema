const fs = require('fs');
let code = fs.readFileSync('src/themes/MinangElegantTheme.jsx', 'utf-8');

// The goal is to move the definitions of the inner components outside.
// We can just use string replacements.
let newCode = code.replace("export default function MinangElegantTheme({", 
`// --- UTILITAS & WARNA ---
const colors = {
  bg: '#1a0f0a',
  text: '#f5ead0',
...
// Now we need to remove all the inner definitions inside MinangElegantTheme.
// The easiest way is to extract the beginning of the function up to handleOpen, and then the return statement.
const handleOpenMatch = code.match(/const handleOpen = \(\) => {[\s\S]*?}/);
if (handleOpenMatch) {
  const handleOpenStr = handleOpenMatch[0];
  
  // We want everything from the start of the file until the end of the return statement.
  // Wait, we can just replace everything from "  // Palet Warna Minang" up to "  return ("
  const startToRemove = newCode.indexOf("  // Palet Warna Minang");
  const endToRemove = newCode.indexOf("  return (\n    <InvitationLayout");
  
  if (startToRemove !== -1 && endToRemove !== -1) {
    newCode = newCode.slice(0, startToRemove) + 
      "  " + handleOpenStr + "\n\n" + 
      newCode.slice(endToRemove);
  }
}

// Now replace the component tags with the ones passing props
newCode = newCode.replace('<CoverSection key="cover" />', '<CoverSection key="cover" bride={bride} groom={groom} primaryEvent={primaryEvent} handleOpen={handleOpen} animateClose={animateClose} />');
newCode = newCode.replace('<ProfileSection />', '<ProfileSection data={data} />');
newCode = newCode.replace('<CountdownSection />', '<CountdownSection countdown={countdown} primaryEvent={primaryEvent} groom={groom} bride={bride} />');
newCode = newCode.replace('<EventsSection />', '<EventsSection akadEvent={akadEvent} baralekEvent={baralekEvent} />');
newCode = newCode.replace('<LoveStorySection />', '<LoveStorySection data={data} />');
newCode = newCode.replace('<GallerySection />', '<GallerySection data={data} />');
newCode = newCode.replace('<WishRsvpSection />', '<WishRsvpSection data={data} wishes={wishes} onSubmitWish={onSubmitWish} />');
newCode = newCode.replace('<FooterSection />', '<FooterSection bride={bride} groom={groom} />');

fs.writeFileSync('src/themes/MinangElegantTheme.jsx', newCode);
